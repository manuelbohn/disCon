...
var posDist = shuffle([[5, 1, 0], [5, 1, 0], [2, 2, 2], [5, 1, 0], [5, 1, 0], [2, 2, 2]]);

//distribution for each trial
var trainingDist = new Array();

// when distribution is [5, 1, 0], the order is set to 5x more frequent - 1x less frequent item
for (var i=0; i < trials.length; i++) {
    trainingDist.push([]);
    if (posDist[i][0] == 5) {
        for (var j=0; j < 5; j++) {
            trainingDist[i].push(trialTargets[i][0])
        }
        trainingDist[i].push(trialTargets[i][1]);
    } else {
