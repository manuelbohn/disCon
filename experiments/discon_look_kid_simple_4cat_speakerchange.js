...
var preloadAudios = ["hi", "lets", "intro", "thank", "it", "needtogo", "imback"];

var posAgents = shuffle(["Bear", "Cat", "Elephant", "Frog", "Sheep", "Tiger", "Mouse"])
...
for (i = 0; i < posAgents.length; i++) {
    for (j = 0; j < preloadAudios.length; j++) {
        var audio = new Audio();
        audio.src = "sounds/speakerchange/" + preloadAudios[j] + "_" + posAgents[i] + ".mp3"
        audios.push(audio);
    }
    for (k = 0; k < preloadItems.length - 3; k++) {
        var audio = new Audio();
        audio.src = "sounds/speakerchange/" + preloadItems[k] + "_" + posAgents[i] + ".mp3"
        audios.push(audio);
    }
}
...
var speakerChange = shuffle(["change", "change", "same", "same"])
...
var experiment = {
    ...
    intro: function () {
        ...
        sourceSound("sounds/speakerchange/" + "hi_" + trainingAgents[trials[0]] + ".mp3");
        playSound();

        sound = document.getElementById("sound");

        sound.onended = function() {
            sourceSound("sounds/speakerchange/" + "intro_" + trainingAgents[trials[0]] + ".mp3");
            playSound();
            sound = document.getElementById("sound");
            sound.onended = function() {
    ...
    train2 : function() {
        ...
        sourceSound("sounds/speakerchange/" + correctItem + "_" + trainingAgents[trials[0]] + ".mp3");
        playSound();
        ...
        sound.onended = function() {
            $(".item").click(function() {
                ...
                data = {
                    subid: subid,
                    subage: subage,
                    experiment: "distribution_kids_simple_speakerchange",
                    trial: trials[0] + 1,

                    agent: trainingAgents[trials[0]],
                    phase: "training",
                    slide: experiment.slides[0],
                    speakerChange:experiment.speakerChange[0],
...
//                setTimeout(function() {
    ...
    choice2 : function() {
        ...
        if (experiment.speakerChange[0] == "change"){
            showAgent(alternativeAgents[0], "straight");
            sourceSound("sounds/speakerchange/" + "it_" + alternativeAgents[0] + ".mp3");
            playSound();
        } else {
            showAgent(trainingAgents[trials[0]], "straight");
            sourceSound("sounds/speakerchange/" + "it_" + trainingAgents[trials[0]] + ".mp3");
            playSound();  
        }
    ...
    transition: function() {
        ...
        if (experiment.speakerChange[0] == "change") {
            ...
            sourceSound("sounds/speakerchange/" + "thank_" + alternativeAgents[0] + ".mp3");
            playSound();
            
        } else {
            showAgent(trainingAgents[trials[0]], "transition");
            sourceSound("sounds/speakerchange/" + "thank_" + trainingAgents[trials[0]] + ".mp3");
            playSound();
        }
