...
```
#Check how many of the participants choosing correct in the no change condition is affected in the speaker change condition
```{r plot2}
pull <- data %>%
  filter(phase == 'test') %>%
  group_by(distribution, workerid, speakerChange) %>%
  summarise(correct = mean(correct_target1),
            correct2 = mean(correct_target2),
            correct3 = mean(correct_target3)) %>%
  filter(speakerChange == "same" & correct > 1/3) %>%
  pull(workerid)

plot2 <- data%>%
  filter(workerid %in% pull) %>%
  filter(phase == 'test') %>%
  group_by(distribution, workerid, speakerChange) %>%
  summarise(correct = mean(correct_target1),
            correct2 = mean(correct_target2),
