...
files <- dir("../../pilot_1_raw_data/production-results_framing/")

raw_data <- data_frame()
for (f in files) {
  jf <- paste("../../pilot_1_raw_data/production-results_framing/",f,sep="")
...
ggplot() +
  geom_jitter(data = plot3, aes(x = item, y = prop_correct, col = distribution, alpha = .2), width = .3,height = .025)+
  geom_pointrange(data = plot3a, aes(x = item, y = mean, col = distribution, ymin = ci_lower, ymax = ci_upper),size = .8)+
  facet_grid(~distribution)+
  geom_hline(yintercept = 1/3, lty=2)+
...
```
# Does where (5,1,0) occurs determine the strategy of chosing the last item?
```{r}
plot3 <- data %>%
  filter(phase == 'test', distribution == "c(5, 1, 0)") %>%
  mutate(distType = ifelse(trial < 4, "early", "late")) %>%
  group_by(distribution, workerid, distType) %>%
  summarise(correct = mean(correct_target1),
            correct2 = mean(correct_target2),
            correct3 = mean(correct_target3)) %>%
  gather(item, prop_correct, -distribution, -workerid, -distType) %>%
  group_by(distribution, item, distType) 
...
plot2 <- data %>%
  filter(phase == 'test') %>%
  group_by(distribution, trialType) %>%
  summarise(correct = mean(correct_target1)
            ) %>%
  gather(item, prop_correct, -distribution, -trialType) %>%
  arrange(workerid)
