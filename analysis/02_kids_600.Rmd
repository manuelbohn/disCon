...
chance_data <- data %>%
  ...
  group_by(subage,subid) %>%
  summarise(correct = mean(correct_target1)) %>%
  mutate(comp_chance= ifelse(correct<1/3,0,1))

library(brms)


chance_model <- brm(comp_chance ~ 1,
                    data = chance_data, family = bernoulli(),
...
pos_plot_mean <- pos_plot %>%
  group_by(pickPos) %>%
  multi_boot_standard(col = "correct")


ggplot() +
  geom_jitter(data = pos_plot, aes(x = pickPos, y = correct,  alpha = .2, col = pickPos),width = .2,height = .01)+
  geom_pointrange(data = pos_plot_mean, aes(x = pickPos, y = mean,  ymin = ci_lower, ymax = ci_upper, col = pickPos),size = .8)+
  geom_hline(yintercept = 1/3, lty=2)+
  labs(x="",y="Proportion correct")+
  theme_few() +
  ylim(-0.05,1.05)+
