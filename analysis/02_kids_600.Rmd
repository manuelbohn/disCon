...
raw_data <- raw_data %>%
  ...
  mutate(subage = str_replace(subage," ",""))
...
log <- read_excel("../../DisCon-subject_log.xlsx", 1)%>%
  mutate(subid = ifelse(subid == "190416_4_d6v1","190416_4_d600",subid),
         subid = ifelse(subid == "190828_3_d500","190828_3_d600",subid))%>%
...
data_ex1 <- data %>%
  ...
  mutate(correct = ifelse(correct_target1 == "1", 1, 0),
         ...
         speaker = agent,
         age_group = subage)%>%
  select(id, age_group,age_num,sex,experiment,trial,speaker,target_category, picked_category,correct)


write_csv(data_ex1, path="../data/data_ex1.csv")
```
...
ggplot(data = p5, aes(x = age_num, y = correct)) +
  ...
  geom_smooth(method = "glm",  se = T,  alpha = .5, size = 1, col = "darkred", fill = "darkgreen", alpha = .5 )+
  geom_hline(yintercept = 1/3, lty=2)+
...
post_plot <- ggplot(plot_dens, aes(posterior, group = factor(age))) +
...
data_frame(
  ...
  WAIC = c(child_waic$loos$kbm1$waic,child_waic$loos$kbm2$waic),
  SE = c(child_waic$loos$kbm1$se_waic,child_waic$loos$kbm2$se_waic),
