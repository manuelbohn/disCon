...
```{r recency_plot3, fig.height = 5, fig.width = 10, echo = FALSE}
recency_plot3 <- data_recency %>%
  filter(phase == 'test') %>%
  group_by(distribution, workerid, experiment) %>%
  summarise(correct = mean(correct_target1),
            correct2 = mean(correct_target2),
...
recency_plot3a <- recency_plot3 %>%
  multi_boot_standard(col = "prop_correct")

ggplot() +
  geom_jitter(data = recency_plot3, aes(x = item, y = prop_correct, col = item, alpha = .2), width = .3,height = .025)+
  geom_pointrange(data = recency_plot3a, aes(x = item, y = mean, col = item, ymin = ci_lower, ymax = ci_upper),size = .8)+
