...
p1 <- data %>%
  filter(phase == 'test')%>%
  group_by(subage, distribution,subid) %>%
  summarise(correct = mean(correct_target1),
            sd = sd(correct_target1)) 

p2 <- p1 %>%
  group_by(subage,distribution) %>%
  multi_boot_standard(col = "correct")


ggplot() +
  geom_jitter(data = p1, aes(x = distribution, y = correct, col = subage), alpha = .2 ,width = .2,height = .01)+
  geom_pointrange(data = p2, aes(x = distribution, y = mean,  ymin = ci_lower, ymax = ci_upper, col = subage),size = .8, position = position_dodge(width = .5))+
  geom_hline(yintercept = 1/3, lty=2)+
...
kbm1 <- brm(correct_target1 ~ age_num * distribution +
                      (distribution| subid) + (distribution | target1),
...
```{r, include = F}
kbm2 <- brm(correct_target1 ~ age_num +
                      (distribution| subid) + (distribution | target1),
                    data = model_data, family = bernoulli(),
