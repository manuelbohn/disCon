...
files <- dir("../../kids_raw_data/6v1/")

raw_data <- data_frame()
for (f in files) {
#  jf <- paste("../../kids_simple_4cat/",f,sep="")
  jf <- paste("../../kids_raw_data/6v1/",f,sep="")
...
ggplot() +
  geom_jitter(data = p1, aes(x = distribution, y = correct, col = distribution), alpha = .2 ,width = .2,height = .01)+
  geom_pointrange(data = p2, aes(x = distribution, y = mean,  ymin = ci_lower, ymax = ci_upper, col = distribution),size = .8)+
...
kbm1 <- brm(correct_target1 ~ age_num * distribution
                      (condition| subid) + (condition | target1),
