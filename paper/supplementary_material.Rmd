---
title             : "Category based discourse inferences in early childhood  \nSupplementary material"
shorttitle        : " "
...
The list below shows all words that were used to request objects during training rounds for each of the four categories. The corresponding pictures are shown in figure 1 in the main manuscript. 
...
bf_t_age_prior <- ex1_data %>%
  ...
  mutate(mean= mean(unlist(correct)),
         default = round(extractBF(ttestBF(unlist(correct), mu = 1/3))$bf,2),
         wide = round(extractBF(ttestBF(unlist(correct), mu = 1/3, rscale = "wide"))$bf,2),
         ultrawide = round(extractBF(ttestBF(unlist(correct), mu = 1/3,rscale = "ultrawide"))$bf,2),
         Age = as.character(age_group))%>%
  ungroup()%>%
  select(Age, default, wide, ultrawide)
``` 
...
# exs_m1 <- brm(correct ~ age_num * condition + (condition|id) + (condition|speaker),
...
# 
# exs_m2 <- brm(correct ~ age_num + condition + (condition|id) + (condition|speaker),
#                     data = exs_model_data, family = bernoulli(),
#           control = list(adapt_delta = 0.99),
#           sample_prior = F,
#           chains = 4,
...
# 
# 
# exs_m3 <- brm(correct ~ age_num + (condition|id) + (condition|speaker),
#                     data = exs_model_data, family = bernoulli(),
#           control = list(adapt_delta = 0.99),
#           sample_prior = F,
#           chains = 4,
...
Figure \ref{fig:plotS2}A shows that performance differed widely across categories. This visual impression was corroborated in a model comparison. We compared a model with condition to a model without condition as a predictor in the same way as in previous experiments. Table \ref{tab:tableS3} shows that the model including category was clearly favored. For comparisons between categories we set clothes as the reference category. Figure \ref{fig:plotS2}B and \ref{fig:plotS3} show that children chose the object from the same category most often when fruits was the target category. Vehicles and animals (reference category) resulted in intermediate levels of performance while clothes was the most difficult. 
...
# con_m1 <- brm(correct ~ age_num + target_category + (1|id) + (target_category|speaker),
...
# #
# con_m2 <- brm(correct ~ age_num + (1|id) + (target_category|speaker),
#                     data = con_model_data, family = bernoulli(),
#           control = list(adapt_delta = 0.99),
#           sample_prior = F,
#           chains = 4,
...
tcon <- loo_compare(con_m1, con_m2, criterion = "waic")%>%
  as.data.frame() %>%
  tibble::rownames_to_column("Model")%>%
  mutate(
    Model = recode(Model, con_m1 = "correct ~ age + category + RE", 
                  con_m2 = "correct ~ age + RE"),
    weight = exp(elpd_waic) / sum(exp(elpd_waic)),
    weight = round(weight, 3),
    WAIC = round(waic, 2),
...
cp1 <- ggplot()+
  ...
  geom_jitter(data = pc1, aes(x = age_group, y = correct, col = target_category), position = position_jitterdodge(jitter.width = .1,jitter.height = .025, dodge.width = .75 ), alpha = .5)+
...
samples_con <- posterior_samples(con_m1, "^b")%>%
  mutate(sample = 1:length(b_age_num))
...
map_con <- tibble(
  ...
  int = fixef(con_m1)[1,1],
  slope = fixef(con_m1)[2,1],
  clothes = fixef(con_m1)[3,1],
  fruits = fixef(con_m1)[4,1],
  vehicles = fixef(con_m1)[5,1]
)%>%
  mutate(y = ifelse(
...
cp2 <- ggplot() +
  geom_hline(yintercept = 1/3, lty=2)+
  geom_line(data = con_sample, aes(x = age, y = y, col = Category, group = sample), size = .01)+
...
cp3 <- tidy(con_m1, prob = 0.95) %>%
  filter(term == "b_age_num" |
           term == "b_target_categoryfruits" |
           term == "b_target_categoryclothes" |
           term == "b_target_categoryvehicles")%>% 
  mutate(term = str_remove(term,"b_"),
         term = str_remove(term,"target_category"),
         term = str_remove(term,"_num"))%>%
...
```{r plotS3, include = T, fig.align = "center", fig.cap = "Model estimates for each category with clothes as the reference category. Dots show mean and error bars 95\\% credible intervals of the posterior distribution.", out.width = '60%', fig.asp = .5}
cp3
```
